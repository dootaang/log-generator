...
class CharacterCardHandler:
    ...
    def _process_ccv3_assets(self):
        ...
        try:
            ...
            for asset in assets:
                try:
                    asset_name = asset.get("name", "").strip()  # 감정 이름
                    asset_uri = asset.get("uri", "")
                    print(f"Processing asset: {asset_name} with URI: {asset_uri}")
                    
                    if asset_uri and ':' in asset_uri:
                        asset_number = asset_uri.split(':')[1]
                        key = f"chara-ext-asset_:{asset_number}"
                        # 이 부분이 중요: image_data에 실제로 있는 키를 확인
                        if key in self.image_data:
                            self.image_uri_map[asset_name] = key
                            print(f"Successfully mapped: {asset_name} -> {key}")
                        else:
                            print(f"Warning: No image data found for {key}")
                except Exception as e:
                    print(f"Error processing individual asset: {e}")
            
            print(f"Processed {len(self.image_uri_map)} assets successfully")
...
class ModernLogGenerator(QMainWindow):
    def __init__(self):
        ...
        self.card_handler = CharacterCardHandler()  # 캐릭터 카드 핸들러 초기화
        self.asset_name_img_tag_map = {}  # 이미지 태그 매핑 저장용
    ...
    def init_ui(self):
        ...
        main_layout.setSpacing(STYLES['spacing_normal'])  # 간격 추가
        
        # === Splitter 추가 ===
        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setHandleWidth(12)  # 핸들 너비 증가
        splitter.setStyleSheet("""
            ...
            QSplitter::handle:horizontal::after {
                background-color: #cccccc;
                content: "";
                position: absolute;
                left: 5px;  /* 실제 선의 위치 조정 */
                width: 2px;  /* 실제 선의 너비 */
                top: 0;
                bottom: 0;
            }
            QSplitter::handle:hover::after {
                background-color: #999999;
            }
            QSplitter::handle:pressed::after {
        ...
        left_panel.setMinimumWidth(300)  # 최소 너비
        left_panel.setMaximumWidth(600)  # 최대 너비 추가
        
        # === 오른쪽 패널 (입출력) ===
        right_panel = self.create_io_panel()
        right_panel.setMinimumWidth(400)  # 최소 너비
        ...
        # splitter 크기 비율 설정
        splitter.setStretchFactor(0, 0)  # 왼쪽 패널은 고정 크기 유지
        splitter.setStretchFactor(1, 1)  # 오른쪽 패널이 나머지 공간 차지
        
        # 초기 크기 설정
        splitter.setSizes([400, 800])  # 초기 크기를 [왼쪽패널, 오른쪽패널]로 설정
    ...
    def create_asset_settings(self):
        ...
        group.addWidget(self.image_url_container)
        ...
        group.addWidget(self.asset_status)
    ...
    # 기존 process_image_tags 메서드를 이것으로 교체
    def process_image_tags(self, content):
        ...
        def replace_tag(match):
            ...
            if tag in url_mappings:
                url = url_mappings[tag]
                return f'''
                    <div style="margin-bottom:1.5rem;text-align:center;">
                        <img style="border-radius: 30px; width: 250px;" src="{url}" class="fr-fic fr-dii">
                        <br>
                    </div>
                '''
            print(f"Warning: No URL mapping found for image: {tag}")
    ...
    def process_image_tags(self, content):
        ...
        def replace_tag(match):
            ...
            if tag in url_mappings:
                url = url_mappings[tag]
                # 이미지를 포함하는 div를 별도의 문단으로 처리
                return f'''</div>
                    <div style="margin: 1rem auto; width: 100%; text-align: center;">
        ...
        # 먼저 content를 div로 감싸기
        content = f'<div style="margin-bottom:1.5rem;">{content}</div>'
    ...
    def convert_text(self):
        ...
        try:
            ...
            for paragraph in input_text.strip().split('\n\n'):
                if paragraph.strip():
                    # 각 문단에 대해 이미지 처리와 대화문 포맷팅
                    processed_para = self.process_image_tags(paragraph)
                    if not processed_para.startswith('<div'):
                        formatted_text = self.format_conversation(processed_para)
                        processed_para = f'<div style="margin-bottom:1.5rem;">{formatted_text}</div>'
                    paragraphs.append(processed_para)
    ...
    def add_image_url_entry(self):
        ...
        if hasattr(self, 'asset_status'):
            self.asset_status.setText(f"아카라이브 글쓰기에서 이미지 HTML 코드를 이미지 URL에 붙여넣습니다.")
    ...
    def convert_text(self):
        try:
            ...
            print("After image tag processing:", content)
            ...
            for paragraph in input_text.strip().split('\n\n'):
                if paragraph.strip():
                    # 이미지 태그 처리 추가
                    formatted_text = self.format_conversation(self.process_image_tags(paragraph))
