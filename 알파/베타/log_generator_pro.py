...
class TemplateManager:
            ...
            if os.path.exists(self.templates_file):
                with open(self.templates_file, 'r', encoding='utf-8') as f:
                    ...
                    for name, template in user_templates.items():
                        if name not in self.templates:  # 기본 템플릿 보호
                            self.templates[name] = template
        except Exception as e:
            print(f"사용자 템플릿 로드 중 오류: {str(e)}")

    def save_user_templates(self):
        """사용자 정의 템플릿만 저장"""
        try:
            # 기본 템플릿을 제외한 사용자 템플릿만 저장
    ...
    def auto_save_current_template(self):
        """현재 설정을 커스텀 템플릿으로 자동 저장"""
        try:
            self.templates["커스텀"] = {
                "name": "커스텀",
                "theme": {
...
class ModernLogGenerator(QMainWindow):
    def __init__(self):
        ...
        self.auto_save_timer.timeout.connect(self.template_manager.auto_save_current_template)
    ...
    def show_template_preview_dialog(self):
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                # 선택된 템플릿이 있으면 적용
                selected_items = dialog.template_list.selectedItems()
                if selected_items:
            ...
            self.handle_error(f"템플릿 프리뷰 중 오류 발생: {str(e)}")

    def create_template_button(self):
        """템플릿 관리 버튼 생성"""
        button = ModernButton("템플릿 관리", primary=True)
        button.setFixedWidth(120)
        button.clicked.connect(self.show_template_menu)
        return button
    ...
    def show_template_menu(self):
        ...
        # 현재 설정을 템플릿으로 저장
        save_action = menu.addAction("현재 설정을 템플릿으로 저장")
        save_action.triggered.connect(self.template_manager.save_current_template)
        
        if self.template_manager.templates:
            menu.addSeparator()
            
            # 템플릿 삭제 메뉴
            delete_menu = menu.addMenu("템플릿 삭제")
            for name in sorted(self.template_manager.templates.keys()):
                delete_action = delete_menu.addAction(name)
                delete_action.triggered.connect(lambda checked, n=name: self.template_manager.delete_template(n))
    ...
    def create_preset_button(self):
        """프리셋 관리 버튼 생성"""
        preset_btn = ModernButton("프리셋/템플릿 관리")
    ...
    def show_preset_menu(self):
        ...
        # === 템플릿 관련 메뉴 (새로 추가) ===
        menu.addSeparator()
        template_menu = menu.addMenu("템플릿 관리")
        
        # 현재 설정을 템플릿으로 저장
        save_template_action = template_menu.addAction("현재 설정을 템플릿으로 저장")
        save_template_action.triggered.connect(self.save_current_template)
        
        template_menu.addSeparator()
        
        # 템플릿 내보내기/가져오기
        export_action = template_menu.addAction("템플릿 내보내기")
        export_action.triggered.connect(self.export_templates)
        
        import_action = template_menu.addAction("템플릿 가져오기")
    ...
    def create_template_settings(self):
        ...
        # 템플릿 관리 버튼
        template_btn = ModernButton("템플릿 관리")
        template_btn.setFixedWidth(120)
        template_btn.clicked.connect(self.show_template_menu)
    ...
    def add_new_tag(self, tag_data=None):
        ...
        # 태그 변경 시그널 연결
        entry.tagChanged.connect(self.update_preview)
    ...
    def save_tag_preset(self):
        ...
        if ok and name:
            ...
            preset_data = {
                'tags': tags,
                'alignment': self.tag_alignment.currentText(),
                'spacing': self.tag_spacing.value()
            }
    ...
    def load_tag_preset(self, name):
        ...
        # 전역 설정 적용
        self.tag_alignment.setCurrentText(preset_data.get('alignment', '가운데'))
        self.tag_spacing.setValue(preset_data.get('spacing', 8))
                ...
                try:
                    ...
                    if self.show_tags.isChecked():
                        try:
                            ...
                            tag_alignment = alignment_map.get(self.tag_alignment.currentText(), "center")
                            tag_spacing = self.tag_spacing.value()
