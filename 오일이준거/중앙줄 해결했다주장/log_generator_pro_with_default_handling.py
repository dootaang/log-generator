...
class ModernLogGenerator(QMainWindow):
    def __init__(self):
        ...
        self.settings = QSettings('YourCompany', 'LogGenerator')  # settings 객체 생성
        self.splitter = None  # splitter 참조를 저장할 변수
        self.init_ui()
        self.restore_geometry_and_state()  # 저장된 상태 복원
    ...
    def init_ui(self):
        ...
        # 메인 레이아웃에 추가
        main_layout = QVBoxLayout()  # main_layout 초기화
    ...
    def restore_geometry_and_state(self):
        ...
        print('Restoring window geometry:', geometry)
        
        if geometry is None:
            print('Geometry not found, applying default geometry.')
            self.setGeometry(100, 100, 1200, 800)
        else:
        ...
        # Splitter 상태 복원
        splitter_state = self.settings.value('splitter_state')
        print('Restoring splitter state:', splitter_state)  # 이 부분에 초기화 추가
        if splitter_state and self.splitter:  # splitter가 초기화된 후에만 실행
            self.splitter.restoreState(splitter_state)
        elif self.splitter:
            # Splitter의 기본 분할 비율 설정
            total_width = self.width()
            self.splitter.setSizes([total_width // 3, (total_width * 2) // 3])
    ...
    def closeEvent(self, event):
        """프로그램 종료 시 상태 저장"""
        try:
            print('Saving window geometry:', self.saveGeometry())
            # 창 크기와 위치 저장
            self.settings.setValue('window_geometry', self.saveGeometry())
            print('Saving splitter state:', self.splitter.saveState())
